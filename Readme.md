...
<h1>Предисловие:</h1>
<h3>Реализовал получение токена без селениума, так же изменился способ получения из json информацию о твитах,
поэтому <code>elonmusk.py</code> - НЕ РАБОТАЕТ, используем <code>twitter.py</code>, так же из __init__ 'headers' можно
переместить в отдельный json, но мне тяжко немного работать с ноутбука, без своего пк =) Поэтому могу заняться этим чуть-чуть
попозже! </h3>
...
Используются так же рандомные юзер-агенты, токен теперь получаем через requests
